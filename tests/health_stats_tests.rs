...
fn test_health_stats_new_fields_initialize() {
    ...
    let ebpf_lost = stats.ebpf_lost_events.load(std::sync::atomic::Ordering::Relaxed);
    assert_eq!(ebpf_lost, 0);
    
    // Test Error Tracking fields
    let proc_errors = stats.proc_read_errors.load(std::sync::atomic::Ordering::Relaxed);
    assert_eq!(proc_errors, 0);
    
    let parsing_errors = stats.parsing_errors.load(std::sync::atomic::Ordering::Relaxed);
    assert_eq!(parsing_errors, 0);
    
    let perm_denied = stats.permission_denied_count.load(std::sync::atomic::Ordering::Relaxed);
    assert_eq!(perm_denied, 0);
    
    let ebpf_fails = stats.ebpf_init_failures.load(std::sync::atomic::Ordering::Relaxed);
...
fn test_health_stats_render_table_contains_new_sections() {
    ...
    assert!(output.contains("EBPF PERFORMANCE"), "Should contain EBPF PERFORMANCE section");
    assert!(output.contains("ERROR TRACKING"), "Should contain ERROR TRACKING section");
    assert!(output.contains("TIMING BREAKDOWN (ms)"), "Should contain TIMING BREAKDOWN section");
    assert!(output.contains("RESOURCE LIMITS"), "Should contain RESOURCE LIMITS section");
    
    // Check for specific metric names
    assert!(output.contains("ebpf_events_per_sec"), "Should contain ebpf_events_per_sec metric");
    assert!(output.contains("ebpf_lost_events_total"), "Should contain ebpf_lost_events_total metric");
    assert!(output.contains("ebpf_map_usage (%)"), "Should contain ebpf_map_usage metric");
    assert!(output.contains("ebpf_overhead_cpu (%)"), "Should contain ebpf_overhead_cpu metric");
    
    assert!(output.contains("proc_read_errors"), "Should contain proc_read_errors metric");
    assert!(output.contains("parsing_errors"), "Should contain parsing_errors metric");
    assert!(output.contains("permission_denied_count"), "Should contain permission_denied_count metric");
    assert!(output.contains("ebpf_init_failures"), "Should contain ebpf_init_failures metric");
    
    assert!(output.contains("parsing_duration"), "Should contain parsing_duration metric");
    assert!(output.contains("serialization_duration"), "Should contain serialization_duration metric");
    assert!(output.contains("lock_wait_duration"), "Should contain lock_wait_duration metric");
    
    assert!(output.contains("open_file_descriptors"), "Should contain open_file_descriptors metric");
    assert!(output.contains("max_file_descriptors"), "Should contain max_file_descriptors metric");
    assert!(output.contains("fd_usage (%)"), "Should contain fd_usage metric");
    assert!(output.contains("metrics_response_size (KB)"), "Should contain metrics_response_size metric");
...
fn test_thread_safety_of_new_fields() {
    ...
    let open_fds = stats.open_fds.load(std::sync::atomic::Ordering::Relaxed);
    assert!(open_fds >= 10 && open_fds <= 19, "Open FDs should be in range");
