...
pub async fn details_handler(
    ...
    writeln!(out, "entries_per_subgroup:     {}", stats.entries_per_subgroup).ok();
    writeln!(out, "total_subgroups:          {}", stats.total_subgroups).ok();
    writeln!(out, "estimated_ram_bytes:      {}", stats.estimated_ram_bytes).ok();
    writeln!(out, "history_seconds:          {} ({} min)", 
        stats.history_seconds, stats.history_seconds / 60).ok();
    writeln!(out).ok();
    ...
    if let Some(subgroup_name) = params.subgroup {
        if let Some(history) = state.ringbuffer_manager.get_subgroup_history(&subgroup_name) {
            ...
            writeln!(out, "=================================").ok();
            writeln!(out, "{:>19} | {:>10} | {:>10} | {:>10} | {:>8} | {:>10}",
                "Timestamp", "RSS (KB)", "PSS (KB)", "USS (KB)", "CPU %", "CPU Time").ok();
            ...
            for entry in history {
                ...
                writeln!(out, "{:>19} | {:>10} | {:>10} | {:>10} | {:>7.1}% | {:>9.2}s",
                    dt, entry.rss_kb, entry.pss_kb, entry.uss_kb, 
                    entry.cpu_percent, entry.cpu_time_seconds).ok();
            }
        } else {
