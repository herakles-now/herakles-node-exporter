//! Background process cache refresh and related health bookkeeping.
    ...
    V: Fn(&ProcMem) -> u32,
{
    ...
    std::array::from_fn(|idx| {
        sorted
            .get(idx)
            .map(|proc| TopProcessInfo::new(proc.pid, value_fn(proc), &proc.name))
            .unwrap_or_default()
    })
}
...
pub async fn update_cache(state: &SharedState) -> Result<(), Box<dyn std::error::Error>> {
                ...
                match parse_memory_for_process(&entry.proc_path, &buffer_config_snapshot) {
                    Ok((rss, pss, uss)) => {
                        ...
                        let start_time_seconds =
                            parse_start_time_seconds(&entry.proc_path).unwrap_or(0.0);
    ...
    {
        let mut cache = state.cache.write().await;
        cache.processes = results.iter().map(|p| (p.pid, p.clone())).collect();
    ...
    for (key, agg_data) in &aggregated_by_subgroup {
        ...
        let top_rss = extract_top_3(procs, |a, b| b.rss.cmp(&a.rss), |p| (p.rss / 1024) as u32);
        let top_pss = extract_top_3(procs, |a, b| b.pss.cmp(&a.pss), |p| (p.pss / 1024) as u32);
