...
fn compile_ebpf_programs() {
    ...
    let mut clang_args = vec![
        ...
        "-I".to_string(), bpf_src.to_str().unwrap().to_string(),
    ];
    ...
    fn find_libbpf_include_dir() -> Option<String> {
        ...
        if let Some(build_dir) = out_path.ancestors().find(|p| {
            p.file_name().map_or(false, |n| n == "build")
        }) {
            // Find libbpf-sys-* directory
            if let Ok(entries) = std::fs::read_dir(build_dir) {
                for entry in entries.flatten() {
                    let path = entry.path();
                    if path.is_dir() && entry.file_name().to_string_lossy().starts_with("libbpf-sys-") {
                        let include_dir = path.join("out").join("include");
                        if include_dir.exists() {
                            println!("cargo:warning=Found libbpf headers at: {}", include_dir.display());
